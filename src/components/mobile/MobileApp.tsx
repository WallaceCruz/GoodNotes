...
import { effectiveStatus } from "@/lib/board-types";
import { triageDeck } from "@/lib/board/triage";
import { cn } from "@/lib/utils";
import { UserMenu } from "../kanban/UserMenu";
...
export function MobileApp() {
  ...
  const activeCardDone = activeCard ? effectiveStatus(activeCard, columns) === "done" : false;
          ...
          {activeCard && (
            <DeckActions
              ...
              onToggleDone={() => {
                ...
                if (!activeCardDone) {
                  toast.success(`"${activeCard.title || "Nota"}" concluída`, {
                    action: {
                      label: "Desfazer",
                      onClick: () => boardActions.setNoteDone(activeCard.id, false),
                    },
                  });
                }
              }}
