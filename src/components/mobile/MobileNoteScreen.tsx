...
import { PRIORITIES, PRIORITY_ICON, PRIORITY_LABEL, noteAssignees } from "@/lib/board-types";
import { boardActions, useFileColumns } from "@/stores/board";
import { cn } from "@/lib/utils";
import { timeAgo } from "@/lib/date";
import { priorityClass } from "../kanban/note-style";
import { AssigneeSelect } from "../kanban/AssigneeSelect";
import { CategorySelect } from "../kanban/CategorySelect";
import { ChecklistEditor } from "../kanban/ChecklistEditor";
import { DeadlinePicker } from "../kanban/DeadlinePicker";
import { RichNoteEditor } from "../kanban/RichNoteEditor";
import { StatusSelect } from "../kanban/StatusSelect";
import { TagEditor } from "../kanban/TagEditor";
        ...
        <button
          onClick={() => {
            ...
            toast.success(`"${note.title || "Nota"}" excluída`, {
              action: { label: "Desfazer", onClick: () => boardActions.restoreNote(removed) },
            });
          }}
        ...
        <Field label="Prioridade">
          <div className="flex flex-wrap gap-1.5">
            {PRIORITIES.map((priority) => (
              <button
                key={priority}
                onClick={() => onChange({ priority: note.priority === priority ? null : priority })}
                className={cn(
