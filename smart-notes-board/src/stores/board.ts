...
/** Arquivo ativo a partir de um estado qualquer — usado por `board-effects`. */
export const activeFileOf = activeFile;
...
/**
 * Sem arquivo aberto, estes seletores devolvem *a mesma* lista vazia.
 *
 * Cada tela resolvia isso com `?? []` — dez lugares criando um array novo a
 * cada render, o que quebra silenciosamente qualquer `useMemo` que dependa
 * deles. A resposta para "e se não houver arquivo?" passa a existir uma vez só.
 */
const NO_COLUMNS: Column[] = [];
const NO_TAGS: TagDef[] = [];

export const useFileColumns = (): Column[] => useBoard((s) => activeFile(s)?.columns ?? NO_COLUMNS);
export const useFileTags = (): TagDef[] => useBoard((s) => activeFile(s)?.tags ?? NO_TAGS);
