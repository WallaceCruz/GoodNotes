...
export function noteSurface(
  ...
  // O estilo clássico é o único que pinta o fundo por classe em vez de variável
  // CSS, e só quando ninguém escolheu uma cor à mão. Os três cards que desenham
  // notas repetiam esta condição; esquecê-la num quarto deixaria o card sem cor.
  if (appearance.style === "classic" && !appearance.bgColor && !opts.tint) {
    classes.push(noteBg[color]);
  }
