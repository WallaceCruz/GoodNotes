...
import { MEMBERS } from "@/lib/board/model";
...
import { AssigneeStack } from "@/components/note/note-parts";
          ...
          {variant === "cta" ? (
            ...
            >
              <span className="flex items-center">
                {value.length ? (
                  <AssigneeStack names={value} size={size} />
                ) : (
                  <span
                    className={cn(
              ...
              <span className="font-medium">
                {value.length
                  ? `${value.length} responsável${value.length > 1 ? "is" : ""}`
