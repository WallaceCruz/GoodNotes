...
function KanbanColumnBase({
  ...
  // Uma coluna, uma fonte de cor. `column.color` é herança de quando dava para
  // pintar a coluna à mão: pintava a faixa e o cabeçalho, enquanto o corpo e a
  // borda vinham da paleta nativa — três tons na mesma coluna. Onde a paleta
  // manda, ela manda sozinha; a cor à mão segue valendo nas colunas do usuário,
  // que não têm paleta.
  const ownColor = accent ? null : column.color;
  ...
  return (
    ...
    >
      <div className={cn("h-1.5 w-full", ownColor ? noteBg[ownColor] : "bg-transparent")} />

      <div
        className={cn(
          "flex items-center gap-1 border-b px-3 py-2",
          !accent && "border-border",
          ownColor ? noteHeaderBg[ownColor] : undefined,
        )}
